...
# content_bytes = list(map(int, content.encode("utf-8")))
# content_bytes = list(map(lambda x: list(map(int, x.encode("utf-8"))), content))
#
# tokens = [i for i in range(256)]
# stats = [(len(content_bytes), len(tokens))]
# mappings = OrderedDict()
#
# old = sum(len(a) for a in content_bytes)
# for i in range(256):
...
#     _count = pairs[-1][1]
#     content_bytes = replace_sublist(content_bytes, list(to_be_replaced), new_token)
#     stats.append((len(content_bytes), len(tokens)))
#     mappings[new_token] = (to_be_replaced, _count)
#     print(f"'{decode_text([decode_mapping(mappings, new_token)])}' : {_count}")

# print(stats)
# print(mappings)

# new = sum(len(a) for a in content_bytes)
#
# print(f"Old: {old}, New: {new} (now only {(new / old):.2})")

# for k, v in mappings.items():
#     print(f"'{decode_text([decode_mapping(mappings, k)])}' : {v[1]}")

# for k, v in reversed(mappings.items()):
#     content_bytes = rereplace_sublist(content_bytes, v[0], k)
...
class Tokenizer:
    ...
    def tokenize(self, text: str, new_tokens: int) -> list[int]:
        ...
        tokens = [i for i in range(256)]
        self.stats = [(len(content_bytes), len(tokens))]
        self.mappings = OrderedDict()
